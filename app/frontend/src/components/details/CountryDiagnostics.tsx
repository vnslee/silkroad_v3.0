...
const STATUS: Record<StatusKey, { fg: string; bg: string; dot: string; ko: string }> = {
  good: { fg: '#3f7a5c', bg: '#e9f3ee', dot: '#4f8a6d', ko: '양호' },
  mid: { fg: '#a8761f', bg: '#fbf0e6', dot: '#d39a3a', ko: '주의' },
  low: { fg: '#b23b2a', bg: '#fbe9e6', dot: '#cf5340', ko: '제약' },
  none: { fg: '#475569', bg: '#eef1f5', dot: '#94a3b8', ko: '정보' },
}
...
const GATE_ITEMS = [
  { key: '외국인 지분', icon: 'groups', label: '외국인 지분 한도' },
  { key: '외환', icon: 'currency_exchange', label: '외환·배당 송금' },
  { key: '데이터 현지화', icon: 'dns', label: '데이터 현지화' },
  { key: '국가신용등급', icon: 'verified_user', label: '국가신용등급' },
  { key: '라이선스 취득', icon: 'badge', label: '라이선스 취득' },
  { key: '라이선스 체제', icon: 'gavel', label: '라이선스 체제' },
  { key: '금리 상한', icon: 'percent', label: '금리 상한 규제' },
]
...
export function RegulatoryGates({
  ...
  const rows = GATE_ITEMS.map((g) => {
    ...
    return { ...g, value, status, insight: it.insight }
  ...
  return (
    <DiagPanel
      icon="traffic"
      title="진입 규제 신호등"
      hint="외국인 진입·송금·라이선스·금리 규제를 신호등으로 요약 — 양호(진입 자유) · 주의(조건부) · 제약."
    >
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-xs">
                  <span className="font-label-md text-label-md text-on-surface truncate">{r.label}</span>
                  <span
    ...
    <div className="flex flex-col gap-xs">
      <div className="flex items-center justify-between gap-xs">
        ...
        <span className="font-label-md text-label-md font-semibold" style={{ color: s.fg }}>
          ...
          <span className="text-outline font-normal"> 일</span>
        </span>
      </div>
...
const RECOVERY_SCORES = [
  { key: '차량회수 절차 용이성', label: '차량회수 절차 용이성', direction: 'up' as const },
  { key: '추심 규제', label: '추심 규제 강도', direction: 'down' as const },
  { key: '충당금 규정', label: '충당금 규정 성숙도', direction: 'up' as const },
  { key: '연체 분류 기준', label: '연체 분류 기준', direction: 'up' as const },
]
...
export function RecoveryRiskPanel({ items }: { items: DetailItem[] }) {
  ...
  return (
    <DiagPanel
      icon="gavel"
      title="채권 회수·리스크 환경"
      hint="담보 회수·추심·건전성 분류 — 1~5 척도(막대 색=진출 유리도). 회수 소요기간은 짧을수록 유리."
    >
      <div className="flex flex-col gap-md">
        {scores.map((m) => (
          <ScoreBar key={m.key} label={m.label} value={m.value} direction={m.direction} />
        ))}
        {days != null && <DurationBar label="법적 회수 소요기간" days={days} />}
      </div>
    </DiagPanel>
  )
}
...
// ════════════════════════════════════════════════════════════════════════════
const IT_SCORES = [
  { key: '디지털 채널 성숙도', label: '디지털 채널', direction: 'up' as const },
  { key: '신용정보', label: '신용정보(CB) 인프라', direction: 'up' as const },
  { key: '결제·정산 인프라', label: '결제·정산 인프라', direction: 'up' as const },
  { key: '디지털 딜러 성숙도', label: '디지털 딜러', direction: 'up' as const },
  { key: '국외이전 제한', label: '데이터 국외이전 제한', direction: 'down' as const },
]
...
export function ITMaturityPanel({ items }: { items: DetailItem[] }) {
  ...
  return (
    <DiagPanel
      icon="lan"
      title="IT 인프라 성숙도"
      hint="디지털 채널·신용정보·결제·딜러 인프라 성숙도(1~5). 국외이전 제한은 낮을수록 유리."
    >
      <div className="flex flex-col gap-md">
        {scores.map((m) => (
          <ScoreBar key={m.key} label={m.label} value={m.value} direction={m.direction} />
